...
# --- מאגר חומר לימוד (מתוך הקבצים שלך) ---
study_materials = {
    "תרופות והחייאה": {
        "אשלגן (Potassium)": """
        ...
        * [cite_start]**חוק חשוב:** בחולים עם היפומגנזמיה והיפוקלמיה, יש לתקן מגנזיום תחילה, אחרת ההיפוקלמיה תהיה עמידה לטיפול[cite: 10, 11].
        """,
        "אדרנלין (Adrenaline)": """
        **בהחייאה:**
        * [cite_start]מינון: 0.01 mg/kg (שזה 0.1 מ"ל לק"ג בתמיסה של 1:10,000)[cite: 38].
        * [cite_start]מקסימום למנה: 1 mg[cite: 41].
        * [cite_start]ניתן לחזור כל 2 דקות[cite: 42].
        
        **באינהלציה (סטרידור):**
        * [cite_start]מינון: 400 mcg/kg (מקס' 5 מ"ג)[cite: 48].
        """,
        "אדנוזין (Adenosine)": """
        [cite_start]**התוויה:** SVT[cite: 58].
        [cite_start]**אופן מתן:** פוש מהיר + שטיפה מהירה (5-10 מ"ל סליין) בברז הכי קרוב ללב (בגלל זמן מחצית חיים קצר)[cite: 72].
        [cite_start]**מינון:** מנה ראשונה 0.1 mg/kg (מקס' 6 מ"ג). מנה שנייה 0.2 mg/kg (מקס' 12 מ"ג)[cite: 60, 62].
        """,
        "סדציה ואינטובציה": """
        **מידזולם (דורמיקום):** מינון 0.1-0.05 mg/kg. [cite_start]יש להיזהר בתינוקות מתחת לגיל חצי שנה (דיכוי נשימתי)[cite: 96, 100].
        **קטמין:** שומר על רפלקס נשימה ולחץ דם (מתאים לשוק). [cite_start]מינון 1-2 mg/kg[cite: 363, 366].
        **רוקורוניום:** משתק שרירים. מינון 1 mg/kg (RSI). [cite_start]חובה לתת סדציה לפני![cite: 155, 161].
        """
    },
    "מצבי חירום ופרוטוקולים": {
        "ספסיס (Sepsis)": """
        [cite_start]**הגדרה:** חשד לזיהום + SIRS (חום/היפותרמיה, לויקוציטוזיס/פניה, טכיקרדיה/פניאה)[cite: 826].
        
        **טיפול מיידי (תוך שעה):**
        1. [cite_start]גישה ורידית + תרביות (אם לא מעכב אנטיביוטיקה)[cite: 841].
        2. [cite_start]אנטיביוטיקה רחבת טווח (למשל מרופנם 20 mg/kg)[cite: 843].
        3. [cite_start]נוזלים: 20 ml/kg בולוס (עד 60 ml/kg תוך ניטור עומס)[cite: 845].
        4. [cite_start]בדיקת לקטט[cite: 844].
        
        [cite_start]**שוק ספטי עמיד:** אם אין תגובה לנוזלים -> נוראדרנלין/אדרנלין[cite: 848].
        """,
        "שוק היפוולמי": """
        **סימנים:** טכיקרדיה (פיצוי ראשוני), מילוי קפילרי איטי, ירידה במתן שתן. [cite_start]ל"ד יורד רק בשלב מאוחר (סימן מבשר רעות!)[cite: 879, 882].
        [cite_start]**טיפול:** החזר נוזלים אגרסיבי (קריסטלואידים) 20 ml/kg[cite: 906].
        """,
        "טראומת ראש (TBI)": """
        **CPP (לחץ זילוח מוחי):** MAP - ICP. [cite_start]יעד בילדים: סביב 40-50[cite: 1132].
        [cite_start]**טריאדה ע"ש קושינג (עליית ICP):** יתר ל"ד + ברדיקרדיה + נשימה לא סדירה[cite: 1142].
        [cite_start]**טיפול ב-ICP גבוה:** הרמת ראש 30 מעלות, סליין היפרטוני (3%) או מניטול, הנשמה עם PCO2 סביב 35-40 (היפרוונטילציה מתונה)[cite: 1137, 1145].
        """
    },
    "קרדיולוגיה והמטו-אונקולוגיה": {
        "אנפילקסיס": """
        [cite_start]**טיפול קו ראשון:** אדרנלין IM בירך[cite: 933].
        [cite_start]**מינון:** 0.01 mg/kg[cite: 936].
        [cite_start]**טיפול נוסף:** נוזלים, ונטולין, סטרואידים (לא בשלב המיידי)[cite: 941, 946].
        """,
        "Tumor Lysis Syndrome (TLS)": """
        [cite_start]**מאפיינים:** היפראוריצמיה, היפרקלמיה, היפרפוספטמיה, היפוקלצמיה[cite: 720].
        [cite_start]**מניעה:** הידרציה גבוהה + אלופרינול/רסבוריקז[cite: 728, 730].
        [cite_start]**סכנה:** היפרקלמיה שמובילה להפרעות קצב[cite: 738].
        """
    }
}

# --- מאגר שאלות (מתוך הקבצים שלך) ---
all_questions = [
    {
        "topic": "תרופות",
        ...
        "correct": "0.01 mg/kg",
        "explanation": "המינון בהחייאת ילדים הוא 0.01 מ״ג לק״ג (1:10,000). [cite_start]מינון של 0.1 הוא פי 10 ועלול להיות קטלני[cite: 38]."
    },
    {
        "topic": "תרופות",
        ...
        [cite_start]"explanation": "לאדנוזין זמן מחצית חיים קצר מאוד (שניות), לכן חובה לתת בפוש מהיר ושטיפה כדי שיגיע ללב לפני שיתפרק[cite: 72]."
    },
    {
        "topic": "פרוטוקולים",
        ...
        "correct": "20 ml/kg",
        [cite_start]"explanation": "לפי פרוטוקול ספסיס, מתחילים ב-10-20 מ״ל לק״ג איזוטוניים (סליין/הרטמן) תוך הערכה חוזרת[cite: 845]."
    },
    {
        "topic": "טראומה",
        ...
        [cite_start]"explanation": "זהו מנגנון פיצוי מוחי בניסיון לשמור על CPP מול ICP גבוה[cite: 1142]."
    },
    {
        "topic": "תרופות",
        ...
        "explanation": "חוסר במגנזיום מקשה על הכליה לשמור אשלגן. [cite_start]ללא תיקון מגנזיום, ההיפוקלמיה תהיה עמידה לטיפול[cite: 10]."
    },
    {
        "topic": "פרוטוקולים",
        ...
        "correct": "אדרנלין IM",
        "explanation": "אין קונטראינדיקציה לאדרנלין באנפילקסיס. [cite_start]זהו הטיפול מציל החיים היחיד המיידי[cite: 933]."
    }
]
...
with st.sidebar:
    ...
    if not st.session_state.user_info:
        ...
        with st.form("login_form"):
            ...
            # תוספת "זכור אותי"
            remember_me = st.checkbox("זכור אותי (למשך החיבור הנוכחי)")
